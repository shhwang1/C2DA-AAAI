...
def return_dataset_balance_self(args,test=False):
    ...
    train_dataset = Imagelists_VISDA(image_set_file_s, root=root, transform1=data_transforms['train'], transform2=None, dict_path2img=dict_path2img)
    target_dataset_val = Imagelists_VISDA(image_set_file_t_val, root=root, transform1=data_transforms['val'], transform2=None, dict_path2img=dict_path2img)    
    if test:
        ...
        target_dataset_test = Imagelists_VISDA(image_set_file_unl, root=root, transform1=data_transforms['test'], transform2=None, dict_path2img=dict_path2img)
    target_dataset_unl = Imagelists_VISDA_un(image_set_file_unl, root=root, transform=data_transforms['val'],transform2=data_transforms['hard'])
    
    target_dataset = Imagelists_VISDA(image_set_file_t, root=root, transform1=data_transforms['train'], transform2=data_transforms['hard'], dict_path2img=dict_path2img) 
    ...
    labeled_target_loader = torch.utils.data.DataLoader(target_dataset, batch_size=min(bs, len(target_dataset)),
        num_workers=nw, shuffle=True, drop_last=True)
